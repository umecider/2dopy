...
###Constants###
current_location = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
# fixed where it makes the data folder by getting current dir (backend bc it's sql.py) and going up a folder (to 2dopy-main)
# caused discrepencies in where the tasks were stored between running it in term (not in dir) and double clicking on the python file.
# for going up one path from directory https://stackoverflow.com/questions/9856683/using-pythons-os-path-how-do-i-go-up-one-directory

DATABASE = os.path.join(current_location, "data/tasklist.db") #changed to specific folder to ensure that it is able to be run no matter where the python command is called from
...
def initialize_table() -> None:
    ...
    '''
    Path(os.path.join(current_location, "data")).mkdir(parents=True, exist_ok=True) #from https://stackoverflow.com/questions/273192/how-do-i-create-a-directory-and-any-missing-parent-directories
